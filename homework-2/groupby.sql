...
-- 2. из таблицы orders
--идентификатор заказа, идентификатор заказчика, вес и страну отгрузки.
...
-- 3. фамилию, имя и телефон сотрудников,
--у которых в данных отсутствует регион
...
SELECT country, COUNT DISTINCT contact_name) as num_unique
FROM suppliers
GROUP BY country
ORDER BY num_unique DESC;

/* 5. суммарный вес заказов (в которых известен регион)
по странам, но вывести только те результаты, где суммарный вес на страну
больше 2750.
Отсортировать по убыванию суммарного веса
(см таблицу orders, колонки ship_region, ship_country, freight) */

SELECT ship_country, SUM(freight) as total_freigtht
FROM orders
